...
export type CardRarity = 'BRONZE' | 'SILVER' | 'GOLD' | 'PLATINUM' | 'MYTHIC';
export type CardType = 'CREATURE' | 'EVENT' | 'ARTIFACT' | 'SOUNDTRACK';
export type CardFormat = 'SINGLE' | 'ALBUM' | 'EP' | 'SOUNDTRACK' | 'FEATURE' | 'REMIX' | 'LIVE' | 'ACOUSTIC' | 'REMASTER' | 'COMPILATION';
...
export interface MasterCardTemplate {
  ...
  genre: string;
  trackNumber?: number;
  previewUrl?: string; // ← Agregado
  artworkUrl: string;   // Asegurado
  themeColor?: string;
  videoId?: string;
  lyrics?: string;
  ...
  // METADATOS SOUNDTRACK
  isSoundtrack?: boolean;
  soundtrackData?: {
    mediaType: 'MOVIE' | 'GAME' | 'SERIES' | 'ANIME' | 'BALLET';
    releaseYear: number;
    composers: string[];
    genres: string[];
    duration: number;
  ...
  rarity: CardRarity;
  isMythic?: boolean;            // ← NUEVO
  keywords: Keyword[];

  // HYBRID MASTERS & VARIANTS (GDD v2.5)
  format?: CardFormat;
  popularity?: number;           // Valor de 0 a 100 (antiguo "90%")
  isCanonical?: boolean;         // Si es la versión principal del Master

  // HERENCIA
  heritage?: {
    compositionId: string;
    heritageRole: 'MASTER' | 'VARIANT';
    masterRarity: CardRarity;    // Rarity heredada
    allVariants?: {
      id: string;
      type: CardType;
      format: CardFormat;
...
export enum Keyword {
  ...
  HYPE_ENGINE = 'hype_engine',
  FALSETTO = 'falsetto',
  FEATURING = 'featuring',
  DISS_TRACK = 'diss_track',
  SAMPLE = 'sample',
  FRENZY = 'frenzy',
  AUTOTUNE = 'autotune',
...
export type ValidCardRarity = 'BRONZE' | 'SILVER' | 'GOLD' | 'PLATINUM' | 'MYTHIC';
