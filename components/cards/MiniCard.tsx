...
export default function MiniCard({ data, className = '' }: MiniCardProps) {
  ...
  const artSrc = data.artUrl?.replace('http://', 'https://') || '';

  return (
    <div
      className={`relative aspect-[2.5/3.5] ${cardBg} rounded-xl overflow-hidden shadow-2xl border-2 ${rarityBorder} group transition-all duration-300 ${className}`}
    >
      {/* Art */}
      ...
      <div className="absolute bottom-1.5 left-1.5 right-1.5 z-10">
        ...
        <p className="text-[8px] text-gray-400 truncate tracking-tight uppercase font-medium mb-1">{data.artist}</p>
