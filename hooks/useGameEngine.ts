...
import { checkCoincidenceBonus } from '@/lib/engine/sharedPlaylistService';
...
export function hasKw(card: CardData, keyword: Keyword | string): boolean {
  if (!card.abilities) return false;
  // Check both abilities and direct keywords
  const inAbilities = card.abilities.some(a => a.keyword === keyword);
  const inKeywords = card.keywords?.includes(keyword as Keyword);
  return inAbilities || inKeywords || false;
}
...
export function useGameEngine() {
  ...
  const startGame = useCallback((playerDeck: CardData[], botDeck: CardData[]) => {
    ...
    const beatDrop: CardData = {
      ...
      rarity: 'BRONZE',
      abilities: [{
        id: 'beat_drop_ability',
        trigger: 'intro' as any,
        effect: 'hype' as any,
        target: 'self' as any,
        value: 1,
        text: 'Beat Drop: El primer jugador gana 1 de Hype extra.',
        mechanicTags: ['coin'],
        statPenalty: 0,
        keyword: Keyword.DROP,
        description: 'Beat Drop (Coin)'
      } as any],
      name: 'Beat Drop',
      artist: 'Tempo',
      album: 'Utility',
      genre: 'Utility',
      artworkUrl: 'https://is1-ssl.mzstatic.com/image/thumb/Music115/v4/91/33/c8/9133c8bf-53a5-10eb-0649-bf6b0e980dd2/source/100x100bb.jpg',
      themeColor: '#eab308',
  ...
  const activateBackstage = useCallback((owner: PlayerKey, backstageIdx: number) => {
    ...
    setSt((prev: PlayerState) => {
      ...
      const isReactivation = card.type !== 'EVENT' && card.type !== 'SOUNDTRACK';
  ...
  const advancePhase = useCallback(() => {
    ...
    switch (currentPhase) {
      case TurnPhase.START:
        ...
        break;
      case TurnPhase.DRAW:
        // Already draws via timer in advancePhase(START) or similar
        setPhase(TurnPhase.MAIN);
        break;
      case TurnPhase.MAIN:
  ...
  useEffect(() => {
    ...
    if (phase === TurnPhase.DRAW) {
      const timer = setTimeout(() => {
        advancePhase();
      }, 1500); // Give time for draw animation/feeling
      return () => clearTimeout(timer);
    }
  }, [phase, advancePhase]);
  ...
  const promoteCard = useCallback((target: PlayerKey, cardIndex: number) => {
    ...
    setSt((prev: PlayerState) => {
      ...
      const card = prev.hand[cardIndex];
      if (!card) return prev;
      return { ...prev, maxEnergy: prev.maxEnergy + 1, energy: prev.energy + 1, canPromote: false, hand: prev.hand.filter((_, i) => i !== cardIndex), graveyard: [...prev.graveyard, card] };
    });
  }, []);
  ...
  return {
    ...
    playerRef, botRef,
    nextDraws: player.deck.slice(0, 3), // ← NUEVA: Para la cola de robos
  };
}
